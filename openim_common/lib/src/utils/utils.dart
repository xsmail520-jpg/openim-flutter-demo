...
class IMUtils {
  ...
  static Future previewMediaFile(
      ...
      bool onlySave = false}) {
    final source = message.isVideoType
        ? MediaSource(
    ...
    final mb = MediaBrowser(
      ...
      onLongPress: (index) {
        final url = source.url;
        if (url == null || url.isEmpty) return;
        IMViews.openDownloadSheet(url, onDownload: () => saveSource(index));
      },
    );
  ...
  static Future<void> saveImage(BuildContext ctx, String url) async {
    ...
    try {
      ...
      Logger.print('saveImage failed: ${e.runtimeType}');
      Logger.print('saveImage stack: $s');
      IMViews.showToast(StrRes.saveFailed);
    } finally {
      // A failed/expired object URL must not leave EasyLoading covering the
